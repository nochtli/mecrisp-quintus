...
var memory : array[$00200000..$0023FFFF] of byte;
...
begin
  ...
  case addr of
    $10000100: read32 := key;         // UART Data Register
    $10000114: read32 := 64 or 1;     // UART Flags Register
  else
    if (addr < $00200000) or (addr > $0023FFFF) then read32 := 0 else
...
begin
  ...
  case addr of
    ...
    $DABBAD00: begin // Special helper to generate binaries with precompiled sources.
                 ...
                 filewrite(coredump, memory[$00200000], data - $00200000);
                 fileclose(coredump);
                 halt;
               end;

    $10000100: if (data and $FF) <> 13 then emit(data and $FF);  // UART Data Register

  else
    if (addr >= $00200000) and (addr <= $0023FFFF) then
    begin
      memory[addr  ] :=  data         and $FF;
      memory[addr+1] := (data shr  8) and $FF;
...
begin
  ...
  writeln;
  addr := $00200000;
  while (addr < $00200000 + binarysize) do
  begin
    inst := read32(addr);
...
begin
  ...
  fileread(binaryimage, memory[$00200000], binarysize);
